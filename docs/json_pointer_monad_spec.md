...
- A pointer monad’s yyjson_mut_val * is only valid for the lifetime of the originating yyjson_mut_doc.
- Do not use a pointer after `yyjson_mut_doc_free(doc)`.
...
typedef struct jpm_ptr {
    yyjson_mut_doc *doc;    /* owner document; nonnull when valid */
...
- If only document-level helpers exist, start from `yyjson_mut_doc_get_root(doc)`.
